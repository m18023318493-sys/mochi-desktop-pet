...
"%PY_EXE%" -3 -c "import PIL" >nul 2>nul
if errorlevel 1 (
  echo Installing the Pillow image library for custom appearances...
  "%PY_EXE%" -3 -m pip install "Pillow>=10.0"
...
if errorlevel 1 (
  echo Installing the Pillow image library for custom appearances...
  "%PYTHON_EXE%" -m pip install "Pillow>=10.0"
  if errorlevel 1 goto dependency_error
)
...
:dependency_error
echo.
echo Mochi could not install the Pillow image library.
echo Check your internet connection, then run: py -3 -m pip install Pillow
pause
exit /b 1
